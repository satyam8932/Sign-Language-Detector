...
while True:
    _,frame=cap.read()
    count = {
             'a': len(os.listdir(directory+"/hey")),
             'b': len(os.listdir(directory+"/there")),
             'c': len(os.listdir(directory+"/how")),
             'd': len(os.listdir(directory+"/do you")),
             'e': len(os.listdir(directory+"/do")),
    ...
    row = frame.shape[1]
    col = frame.shape[0]
    cv2.rectangle(frame,(0,40),(350,450),(255,255,255),2)
    cv2.imshow("data",frame)
    cv2.imshow("ROI",frame[40:450,0:350])
    frame=frame[40:450,0:350]
    interrupt = cv2.waitKey(10)
    if interrupt & 0xFF == ord('a'):
        cv2.imwrite(directory+'hey/'+str(count['a'])+'.png',frame)
    if interrupt & 0xFF == ord('b'):
        cv2.imwrite(directory+'there/'+str(count['b'])+'.png',frame)
    if interrupt & 0xFF == ord('c'):
        cv2.imwrite(directory+'how/'+str(count['c'])+'.png',frame)
    if interrupt & 0xFF == ord('d'):
        cv2.imwrite(directory+'do you/'+str(count['d'])+'.png',frame)
    if interrupt & 0xFF == ord('e'):
        cv2.imwrite(directory+'do/'+str(count['e'])+'.png',frame)
